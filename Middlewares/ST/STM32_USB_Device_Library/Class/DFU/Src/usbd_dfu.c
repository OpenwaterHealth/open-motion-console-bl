...
#error "ERROR: usbd_dfu_core.c: Modify the file to support more descriptors!"
  ...
  0x0F,                                                /* bmAttribute:
                                       ...
                                       NOTE: ManifestationTolerant is set to 1 so the device
                                       answers the host's post-download GET_STATUS (returning to
                                       dfuIDLE) instead of self-resetting inside DFU_Leave(). This
                                       avoids dfu-util's "Error during download get_status". The
                                       reboot into the new image is instead triggered from the
                                       bootloader main loop once manifestation completes
                                       (see DFU_ImageDownloadComplete() / main.c). */
