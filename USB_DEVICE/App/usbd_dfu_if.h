...
/* Returns 1 once a DFU image has been programmed and the host has completed the
 * manifestation phase; the bootloader main loop uses this to reboot into the
 * freshly flashed image. */
uint8_t DFU_ImageDownloadComplete(void);

/* Anti-rollback (DFU-time). DFU_IsRollback() returns 1 if the just-downloaded
 * image's version is lower than the version installed at DFU entry.
 * DFU_InvalidateImage() erases the slot header so a rejected downgrade can never
 * boot. DFU_ClearDownloadState() resets the completion latch after handling. */
