...
typedef enum
{
  ...
  SB_SYSCALL_SYSTICK_SUSPEND /*!< SB_SFU service call identifier: Systick suspend */
...
/* Disengage SE MPU isolation before control leaves the trusted bootloader in
   the app-privileged model: wipe SE key RAM and disable the MPU. Used on both
   the app-launch and USB-DFU-return paths. No-op when SFU_MPU_PROTECT_ENABLE is
   not defined; not used in the unprivileged-boot model (SFU_MPU_UNPRIV_BOOT). */
