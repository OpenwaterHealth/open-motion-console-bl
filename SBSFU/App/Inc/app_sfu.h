...
#define SFU_DEBUG_MODE               /*!< Enabled when DEBUG_ENABLED is defined (set by CMake for Debug builds).
                                          Comment this define to optimize memory footprint (debug mode removed)
                                          No more print on terminal during SBSFU execution */
#if !defined(DEBUG_ENABLED)
#undef SFU_DEBUG_MODE                /*!< Remove debug output in Release builds */
#endif /* !DEBUG_ENABLED */
...
/**
  ...
  */
#if !defined(SFU_TEST_PROTECTION)
#define SECBOOT_LOADER SECBOOT_USE_NO_LOADER         /*!< No internal/external loader. SBSFU verifies
                                                          and launches the app if valid; returns to main()
                                                          (USB DFU) when no valid firmware is found.
                                                          See sfu_boot.c SFU_BOOT_SM_RebootStateMachine. */
#else
...
/*#define SFU_TAMPER_PROTECT_ENABLE*/ /*!< Disabled: openmotion-bl has no tamper GPIO wiring */
...
/* ---------------------------------------------------------------------------
 * Secure Engine key-RAM wipe (SE isolation, app-privileged model)
 * ---------------------------------------------------------------------------
 * The application runs PRIVILEGED (no app-side changes). In that model the MPU
 * cannot fence a privileged app out of the SE regions, and to launch the app on
 * this board the MPU must be disabled before the jump anyway — so enabling the
 * full MPU isolation machinery (SFU_MPU_PROTECT_ENABLE) buys NO confidentiality
 * here while adding boot-time risk (PRIVDEFENA=0 map, execute-never on the app
 * flash region, launch-path changes). It is therefore intentionally left OFF.
 *
 * What we DO keep is the part that actually protects the keys with no downside:
 * SFU_WIPE_SE_RAM_ON_EXIT zeroizes the SE key RAM (the AES firmware-key copy,
 * the ECC public-key buffer, the AES/SHA contexts, the SE stack) just before
 * control leaves the trusted bootloader — on both the app-launch and the
 * USB-DFU-return paths (see SFU_SE_RAM_Erase() / SFU_MPU_DisengageForExit() in
 * sfu_mpu_isolation.c). This closes the "app / DFU UPLOAD reads leftover key
 * material out of SE RAM" exfil path and changes nothing else vs. the working
 * boot path.
 *
 * Still deferred (needs the option-byte pass):
 *   - The SE *flash* key (se_key.s AES key) stays readable by a privileged app
 *     and via DFU UPLOAD until PCROP (execute-only) is applied to the SE key
 *     region. PCROP is the mechanism that hides the flash key without requiring
 *     the app to be unprivileged.
 *   - Full ST-style app<->SE isolation would require SFU_MPU_PROTECT_ENABLE +
