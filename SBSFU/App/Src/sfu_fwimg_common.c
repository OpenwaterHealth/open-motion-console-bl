...
SFU_IMG_InitStatusTypeDef SFU_IMG_InitImageHandling(void)
{
  ...
  for (i = 0U; i < SFU_NB_MAX_ACTIVE_IMAGE; i++)
  {
...
#endif /* (__GNUC__) */
    {
      if (!IS_ALIGNED(SlotStartAdd[SLOT_ACTIVE_1 + i]))
      {
        ...
        TRACE("\r\n= [FWIMG] SLOT_ACTIVE_%lu (%lx) is not properly aligned\r\n",
  ...
  {
    e_ret_status = SFU_IMG_INIT_FLASH_CONSTRAINTS_ERROR;
    TRACE("\r\n= [FWIMG] magic size (%lu) is not matching the FLASH constraints\r\n", (unsigned long)MAGIC_LENGTH);
  ...
  if (0U != (uint32_t)(SE_FW_HEADER_TOT_LEN % (uint32_t)sizeof(SFU_LL_FLASH_write_t)))
  {
    ...
    e_ret_status = SFU_IMG_INIT_FLASH_CONSTRAINTS_ERROR;
    TRACE("\r\n= [FWIMG] FW Header size (%lu) is not matching the FLASH constraints\r\n", (unsigned long)SE_FW_HEADER_TOT_LEN);
  ...
  for (i = 0U; i < SFU_NB_MAX_ACTIVE_IMAGE; i++)
  {
...
#endif /* (__GNUC__) */
    {
      ...
      {
        TRACE("\r\n= [FWIMG] SLOT_ACTIVE_%lu overlaps SBSFU code area protected by WRP\r\n", i + 1U);
        e_ret_status = SFU_IMG_INIT_FLASH_CONSTRAINTS_ERROR;
      }
    }
  }
  ...
  for (i = 0U; i < SFU_NB_MAX_ACTIVE_IMAGE; i++)
  {
...
#endif /* (__GNUC__) */
    {
      ...
      {
        TRACE("\r\n= [FWIMG] Header of SLOT_ACTIVE_%lu is not under the MPU protection\r\n", i + 1U);
        e_ret_status = SFU_IMG_INIT_FLASH_CONSTRAINTS_ERROR;
      }
    }
  }
...
SFU_ErrorStatus SFU_IMG_LaunchActiveImg(uint32_t SlotNumber)
{
  ...
  /* The Systick interrupt must be disabled before entering the SVC interrupt because if the Systick
  ...
  /* Full-isolation model: SBSFU runs unprivileged and the MPU stays engaged
   * across the jump, so escalate to the privileged SVC handler which performs
   * the secure-user-memory activation and the exception-return jump. The app
   * then runs unprivileged and reaches SE services only through the call gate. */
  SFU_MPU_SysCall((uint32_t)SB_SYSCALL_LAUNCH_APP, SlotStartAdd[SlotNumber] + SFU_IMG_IMAGE_OFFSET);
#else
  /* App-privileged model (this port) and pure development mode: jump to the
   * application directly — the standard bootloader hand-off — instead of the
   * SVC/exception-return path. That path does not reproduce the interrupt /
   * CONTROL / FPU state this board's application needs at reset (it deadlocks in
   * HAL_Delay and/or faults on stale FP context), so it is reserved for the
   * unprivileged model above. The firmware sits at SlotStart+SFU_IMG_IMAGE_OFFSET;
   * vector[0]=initial SP, vector[1]=reset handler. */
  {
    uint32_t app_vector = SlotStartAdd[SlotNumber] + SFU_IMG_IMAGE_OFFSET;
    uint32_t app_sp     = *(volatile uint32_t *)app_vector;
    uint32_t app_pc     = *(volatile uint32_t *)(app_vector + 4U);

#if defined(SFU_WIPE_SE_RAM_ON_EXIT) || defined(SFU_MPU_PROTECT_ENABLE)
    /* Before the hand-off: wipe the SE key RAM (the durable protection in the
     * app-privileged model) and, if MPU isolation is compiled in, disable the
     * MPU so the privileged application runs on a flat memory map. Done while
     * still privileged and before masking interrupts. */
    SFU_MPU_DisengageForExit();
#endif /* SFU_WIPE_SE_RAM_ON_EXIT || SFU_MPU_PROTECT_ENABLE */

    __disable_irq();                 /* mask interrupts only during the transition */
    SCB->VTOR = app_vector;          /* point the vector table at the application */
    __DSB();
    __ISB();
    __set_MSP(app_sp);               /* load the application's initial stack pointer */
    __set_CONTROL(0U);               /* privileged, MSP, no FP active */
    __ISB();
    /* Re-enable interrupts so the application starts exactly like after a normal
     * reset (PRIMASK=0). The bootloader suspended SysTick before this call, so
     * none fire until the application re-initialises and re-enables them. Leaving
     * interrupts masked here deadlocks the app: HAL_Delay() spins on HAL_GetTick(),
     * which only advances from the SysTick interrupt (observed hanging in USB init
     * before reaching the UART output). */
    __enable_irq();
    ((void (*)(void))app_pc)();      /* branch to the application reset handler */
  }
