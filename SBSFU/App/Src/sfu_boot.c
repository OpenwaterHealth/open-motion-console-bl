...
#include "sfu_error.h"
#include "stm32h7xx_it.h" /* required for the HAL Cube callbacks */
/*
...
#include "anti_rollback.h"            /* persistent monotonic anti-rollback floor */
...
/*!
 * Flag set by SFU_BOOT_SM_RebootStateMachine when SECBOOT_USE_NO_LOADER is configured and
 * the boot state machine could not find / verify a valid firmware image.
 * When non-zero the SM loop in SFU_BOOT_SM_Run() exits WITHOUT calling SFU_BOOT_ForceReboot(),
 * allowing SFU_BOOT_RunSecureBootService() to return to main() so that USB DFU can run.
 */
#if (SECBOOT_LOADER == SECBOOT_USE_NO_LOADER)
static uint8_t m_uNeedDownload = 0U;
#endif /* SECBOOT_USE_NO_LOADER */
...
/**
  ...
  */
uint32_t initialDeviceStatusCheck;
...
{
    /* Width is computed inline (not a local) so it produces no unused-variable
     * warning in Release builds, where TRACE() expands to nothing. */
    TRACE(
        "\033c\r\n\r\n"
        "\r\n======================================================================"
...
#if (SECBOOT_LOADER == SECBOOT_USE_NO_LOADER)
  while ((e_ret_status == SFU_SUCCESS) && (m_uNeedDownload == 0U))
#else
...
#if (SECBOOT_LOADER == SECBOOT_USE_NO_LOADER)
  if (m_uNeedDownload != 0U)
  {
    /* No valid firmware was found and no loader is configured.
     * Return to the caller (SFU_BOOT_RunSecureBootService -> main) so that
     * the USB DFU path can accept a new signed firmware image. */
    return e_ret_status;
  }
#endif /* SECBOOT_USE_NO_LOADER */
...
static void SFU_BOOT_SM_CheckStatusOnReset(void)
{
  ...
  /* RCC access done to identify the wakeup sources, we could switch to Unprivileged.
   * We drop to unprivileged execution ONLY for full ST-style isolation
   * (SFU_MPU_UNPRIV_BOOT), which also requires the launched application to be
   * unprivileged-safe. In this port's configuration SFU_MPU_UNPRIV_BOOT is left
   * undefined: the SE key material is instead protected by wiping the SE RAM
   * before the jump (see sfu_mpu_isolation.c), and the bootloader stays
   * privileged so the custom USB-DFU-fallback path (which returns to main() and
   * drives USB + flash programming) does not fault on its privileged accesses.
   * Dropping to unprivileged with no MPU regions programmed would also fault on
   * the first System Control Space access (SysTick/NVIC/SCB). The SVC stub
   * (sfu_se_mpu.s) selects MSP/PSP from EXC_RETURN, so SFU_MPU_SysCall() works
   * from privileged thread mode regardless. */
...
static void SFU_BOOT_SM_CheckUserFwStatus(void)
{
  ...
  if (MASTER_SLOT != 0xFFU)
  {
    if (SFU_SUCCESS == SFU_IMG_DetectFW(MASTER_SLOT))
    {
      ...
      TRACE("\r\n\t  A FW is detected in the slot SLOT_ACTIVE_%lu", m_ActiveSlotToExecute);
    ...
    for (i = 0U; i < SFU_NB_MAX_ACTIVE_IMAGE; i++)
    {
      if (SlotStartAdd[SLOT_ACTIVE_1 + i] != 0U)       /* Slot configured ? */
      {
        if (SFU_IMG_VerifyEmptyActiveSlot(SLOT_ACTIVE_1 + i) != SFU_SUCCESS)
        {
           ...
           */
          TRACE("\r\n\t  Slot SLOT_ACTIVE_%lu not empty : erasing ...", SLOT_ACTIVE_1 + i);
...
#else
    /*
     * No internal/standalone loader and no valid FW. Rather than stopping in a
     * blink loop, route to the reboot/standby state which (in NO_LOADER mode)
     * sets m_uNeedDownload so SFU_BOOT_SM_Run() returns to main() and the USB DFU
     * download path can accept a new signed firmware image.
     */
...
static void SFU_BOOT_SM_ExecuteUserFw(void)
{
  ...
  /* Double security check for all active slots :
     ...
     - Control twice the slot beyond firmware image */
  ...
  /* ---- Anti-rollback enforcement (signature-verified) ----------------------
   * The active image's header signature and FW tag have just been verified
   * above, so its FwVersion is trusted. Refuse to launch a version below the
   * persistent monotonic floor (stored in a DFU-protected flash sector); on a
   * downgrade, invalidate the image so it cannot boot, then reboot — the
   * bootloader then finds no valid FW and falls through to USB DFU so a build
   * with an equal-or-higher version can be flashed. On an accepted version,
   * raise the floor. */
  {
    /* Read the FwVersion directly from the already-verified active-slot header
     * (the double-check above authenticated it). We deliberately do NOT call
     * SFU_IMG_GetActiveFwVersion(): it re-enters the Secure Engine to re-
     * authenticate the header, which corrupts the boot flow-control state and
     * aborts the launch. SFU_LL_FLASH_Read is the SE/MPU-safe read path. */
    uint8_t hdr_buf[SE_FW_HEADER_TOT_LEN];
    SE_FwRawHeaderTypeDef *p_hdr = (SE_FwRawHeaderTypeDef *)(uint32_t)hdr_buf;
    uint16_t active_ver = 0U;

    if (SFU_LL_FLASH_Read(hdr_buf, (uint8_t *)SlotHeaderAdd[m_ActiveSlotToExecute],
                          sizeof(hdr_buf)) == SFU_SUCCESS)
    {
      active_ver = p_hdr->FwVersion;
    }

    if ((uint32_t)active_ver < AntiRollback_GetFloor())
    {
      TRACE("\r\n= [SBOOT] ANTI-ROLLBACK: FW version %u below floor %u - launch refused\r\n",
            (unsigned int)active_ver, (unsigned int)AntiRollback_GetFloor());
      SFU_EXCPT_SetError(SFU_EXCPT_VERSION_ERR);
      (void)SFU_IMG_InvalidateCurrentFirmware(m_ActiveSlotToExecute);
      SFU_BOOT_ForceReboot();   /* does not return */
    }

    /* Accepted: raise the floor (monotonic; no-op if not higher). */
    AntiRollback_UpdateFloor((uint32_t)active_ver);
  }
...
#if (SECBOOT_LOADER == SECBOOT_USE_NO_LOADER)
  /* No loader configured: instead of rebooting, signal the SM loop to exit
   * so SFU_BOOT_RunSecureBootService() returns to main() and USB DFU can run.
   * Before leaving the trusted bootloader, wipe the SE key RAM and disable the
   * MPU so the (privileged) USB-DFU path runs on a flat map and no key material
   * remains reachable via the DFU UPLOAD/read path. No-op without MPU isolation. */
  SFU_MPU_DisengageForExit();
  TRACE("\r\n= [SBOOT] No valid firmware - entering USB DFU download mode\r\n");
  m_uNeedDownload = 1U;
  return;  /* exits SFU_BOOT_SM_RebootStateMachine; loop condition checked next */
#else
  /* This is the last operation executed. Force a System Reset */
  SFU_BOOT_ForceReboot();
#endif /* SECBOOT_USE_NO_LOADER */
