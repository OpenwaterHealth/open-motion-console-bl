...
SFU_ErrorStatus SFU_LL_SECU_CheckApplyRuntimeProtections(uint8_t uStep)
{
  ...
  /* Only read by the verbose trace and the (config-gated) protection blocks
     above; reference it so the build is clean when those are disabled. */
...
SFU_ErrorStatus SFU_LL_SECU_SetProtectionANTI_TAMPER(void)
{
  ...
  /* RTC_TAMPER_3 (PC1) selected. PC13 connected to RTC_TAMPER_1 is also connected to the USER button */
